...
<%-- JSP 주석(comment) --%>
<%-- 
Servlet: Server + Applet(web Server에서 실행되는 작은 애플리케이션)
	- 자바 클래스
...
JSP의 동작 원리: 
	최초 요청(request)이 웹 서버에 도착했을 때, 웹 서버스는 JSP가 컴파일된 서블릿 클래스의 객체가 생성됐는지 검사
...
JSP의 구성 요소(태그)
	1. 지시문(directive): <%@ ... %>
		page, include, taglib등을 설정(JSP의 환경 설정 정보)
	2. 선언문(declaration): <%! ... %>
		JSP가 java 파일로 변환될 때, 멤버 변수, 메서드를 정의 하는 코드
	3. 스크립트릿(scriptlet): <% ... %>
		JSP가 java 파일로 변환될 때, _jspService(resquest, response) 메서드 안에서 포함되는 코드들.
