...
public class AuthenticationFilter implements Filter {
    ...
    public AuthenticationFilter() {
        // TadminODO Auto-generated constructor stub
    }
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) 
		...
		// 로그인 되어 있는 지 체크
		HttpServletRequest httpRequest = (HttpServletRequest) request; // casting 강제타입변환
		HttpSession session = httpRequest.getSession();
		String signInUser = (String) session.getAttribute("signInUser");
		
		if (signInUser != null) { // 세션이 속성인 로그인 정보가 저장된 경우 - > 로그인 되어 있는 경우
			chain.doFilter(request, response);
			// 요청을 다음 필터 체인으로 전달해서 계속 진행.
		} else { // 세션에 속성이 저장되지 않은 경우 -> 로그인 되어 있지 않은 경우
			// chin.doFilter()를 호출하지 않고, 로그인 페이지로 리다이렉트
