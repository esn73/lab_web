...
<html>
	...
	<body>
		...
		EL에서 상태 정보들을 유지하기 위해서 사용하는 객체들:
			...
			* sessionScope: $ {requestScope }. JSP의 session 객체.
			* applicationScope: $ {requestScope }. JSP의 application 객체.
		
			EL의 ${ 변수 } 동작 원리: 
				* EL은 변수 이름을 pageScope - > requestScope -> sessionScope -> applicationScope 순으로 찾음.
				* { 변수 }: JSP 내장 객체(pageContext, request, 
		 --%>
		 ...
		 // requeset 객체에 "number2" 속성 값을 2로 설정
		 ...
		 <h2>EL</h2>
		 <div>
		 page 속성: ${ number1 } <br> <%--${ pageScope.number1 }  --%>
		 request 속성: ${number2 } <br>  <%--${ pageScope.number2 } 찾은 후 => ${requstScope.number2}  --%>
