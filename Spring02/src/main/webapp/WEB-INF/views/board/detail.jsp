...
<body>
   <div class="container-fluid">
         ...
         <form>
            ...
            <div>
               <label for="title">글 제목</label> <input type="text" id="title"
                  name="title" value="${board.title}" required autofocus readonly />
            </div>
            <div>
               <label for="content">글 내용</label>
               <textarea rows="5" id="content" name="content" required readonly>${board.content}</textarea>
            </div>
            <div>
               <label for="userid">작성자 아이디</label> <input type="text" id="userid"
                  name="userid" value="${board.userid}" required readonly />
            </div>
            <div>
      ...
      <div> <%-- 댓글 작성 양식(form) --%>
         ...
         <input type="text" id="reply_userid" name="userid" value="${signInUserId}" readonly/> <%-- 로그인을 해야만 보여져야 하기때문에 --%>
   ...
   $(document).ready(function(){
      ...
      // 게시글 번호(boardNo)에 달려 있는 모든 댓글 목록을 읽어오는 Ajax 함수 정의 jsp에서 배움
      function getAllReplies(){
         ...
         $.getJSON('/ex02/replies/all/' + boardNo, function(respText){
            ...
            $(respText).each(function(){
               var date = new Date(this.regdate); // JavaScript의 Date 객체 생성
               ...
               list += '<div class="reply_item">'
                  ...
                  + this.rtext
                  + '" />'
                  + '<input type="text" id="reply_userid" name="reply_userid" value="'
                  + this.userid
                  + '" readonly />'
      ...
      // 댓글 삭제 버튼
      $('#replies').on('click', '.reply_item .reply_delete', function(){
